...
def validate_adversary_config(config: dict[str, Any]) -> dict[str, Any]:
    ...
    _require(config.get("purpose") == "tiny_training_smoke_only", "unsupported adversary configuration purpose")
    _require(config.get("research_claims_allowed") is False, "smoke config cannot authorize research claims")
    _require(
    ...
    _require(environment.get("victim_slot") == 0, "only the approved slot-0 victim is allowed")
    _require(environment.get("max_controlled_agents") == 1, "exactly one controlled victim is required")
    _require(environment.get("scene_source") == "pinned_smoke_scene", "smoke training must use the pinned scene")
    _require(environment.get("episode_horizon") == 91, "pinned smoke horizon must remain 91")
    ...
    _require(failure.get("scope") == "victim_only", "failure scope must remain victim-only")
    _require(
    ...
    _require(training.get("checkpoint_every_iterations") == 1, "smoke training checkpoints every iteration")
